...
class Httprequest {
	...
	setinterceptors(instance: AxiosInstance, { isLoading, isCancel, unique, isToast, isRepeatedRequests }: IAxiosHelper) {
	 ...
	 * @returns
	 */
	request(axiosConfig: AxiosRequestConfig, { isLoading = true, isCancel = true, isToast = true, isRepeatedRequests = true }: Partial<IAxiosHelper> = {}) {
		...
		if (isRepeatedRequests) {
			...
			// 确定唯一性(路由+接口地址+请求方式)
			unique = `${location.href}${config.method}${config.url}${paramsStr}`.toLocaleUpperCase();
		}

		this.setinterceptors(instance, { isLoading, isCancel, unique, isToast, isRepeatedRequests });
...
export const httpGet = (url: string, params?: object, axiosHelper?: Partial<IAxiosHelper>) => {
	return http.request(
		{
			url,
			params,
...
export const httpPost = (url: string, data?: object, axiosHelper?: Partial<IAxiosHelper>) => {
	return http.request(
		{
			url,
			data,
